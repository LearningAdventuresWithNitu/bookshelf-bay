...
contract Library is Ownable {
    ...
    // Get book details
    function getBookDetails(uint256 _id) public view bookMustExist(_id) returns (string memory, uint256, address[] memory) {
      return (books[_id].title, books[_id].copies, books[_id].borrowers);
    }

    // To check the books the user has borrowed
    function getBorrowedBooks() public view returns (AvailableBook[] memory) {
      uint256 counter = 0;
      for (uint256 i = 0; i < books.length; i++) {
          if (_hasBook(i, msg.sender)) {
              counter++;
          }
      }

      AvailableBook[] memory borrowedBooks = new AvailableBook[](counter);
      counter = 0;
      for (uint256 i = 0; i < books.length; i++) {
          if (_hasBook(i, msg.sender)) {
              borrowedBooks[counter] = AvailableBook(i, books[i].title);
              counter++;
          }
      }
